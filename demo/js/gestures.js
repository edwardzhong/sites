...
function gestures(elem) {
	...
	function touchMove(e) {
		...
		if (fingers.length > 1) {
			...
			var dc = Math.sqrt(
				Math.pow(fingers[1].dx - fingers[0].dx, 2) +
					Math.pow(fingers[1].dy - fingers[0].dy, 2)
			);
			//旋转的弧度
			var rad = Math.atan2(
				fingers[1].dy - fingers[0].dy - (fingers[1].y - fingers[0].y),
				fingers[1].dx - fingers[0].dx - (fingers[1].x - fingers[0].x)
			);
			...
			document.getElementsByClassName(
				"txt"
			)[0].innerHTML = `${scale}:${(rad * 180) / Math.PI}`;
		} else {
			// 单点触控 => 位移
