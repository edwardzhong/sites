...
function factory(type, pos) {
	switch (type) {
		...
		case "dash":
			return new Dash(pos);
		case "quadratic":
			return new Quadratic(pos);
		case "bezier":
			return new Bezier(pos);
		case "triangle":
			return new Triangle(pos);
		case "rect":
			return new Rect(pos);
		case "round":
			return new Round(pos);
		case "polygon":
	...
	update(i, pos) {
		if (i == 9999) {
			...
			if (i == 0) {
				...
				this.b = Math.round(
					Math.sqrt(
						Math.pow(this.points[1].x - this.x, 2) +
							Math.pow(this.points[1].y - this.y, 2)
					)
				);
				this.angle =
					Math.PI / 2 +
...
document.getElementsByClassName("toolbar")[0].addEventListener(
	"click",
	function(e) {
...
document.getElementById("grid").onclick = document.getElementById(
	"control"
).onclick = function(e) {
	drawBG();
	drawGraph();
