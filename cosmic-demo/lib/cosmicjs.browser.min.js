	...
	2: [function(require, module, exports) {
		...
		module.exports = {
			...
			deleteObject: function(config, object, callback) {
				var endpoint = api_url + "/" + api_version + "/" + config.bucket.slug + "/delete-object?write_key="+object.write_key+"&_id="+object._id;
				fetch(endpoint, {
					method: "post",
					headers: {
						"Content-type": "application/json"
			...
			addMedia: function(config, params, callback) {
				...
				if (config.bucket.write_key) form.append("write_key", config.bucket.write_key);
				form.append("media", params.media);
				if (params.folder) form.append("folder", params.folder);
				superagent.post(endpoint).send(form).end(function(err, response) {
					if (response.status >= 400) {
						var err = {
							message: "There was an error with this request."
						};
